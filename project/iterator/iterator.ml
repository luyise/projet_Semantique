...
open! Cfg
open Domain
let display = ref false         (* mettre à true pour une description du calcul lors de l'itération, false pour n'afficher que le résultat de l'analyse *)
let display_minimal = ref false
...
module type ITERATOR = 
  sig
    ...
    val pp_est : Format.formatter -> t -> unit
...
module MakeIterator(D : DOMAIN) : ITERATOR =
  struct
    module D = D
    type env = D.t
    (* Estimation : famille d'environnement indexé par les noeud *)
      ...
      Est.add
        ...
        (
          List.fold_left
            (
              fun (est : t) (x : node) ->
                Est.add x D.bottom(*D.init_bot x_list*) est
            )
            Est.empty
            n_list
        )
    ...
    let iterate_cfg = fun fmt graph ->
      ...
      let is_reachable = Array.make (List.length graph_nodes + 1) false in (* indique si un noeud est accessible ou non afin de ne pas élargir des environnement inutilement *)
      let () = is_reachable.(graph.cfg_init_entry.node_id) <- true in
      ...
      let rec clean (est : t) : (t) = (* fonction récursive qui va mettre à jour l'estimation *)
        if Queue.is_empty dirty then est else begin
          ...
          let _ = if !display_minimal || !display then Format.fprintf fmt "noeud courant : %i : \n" current_node.node_id in

          match current_node.node_in with (* on regarde si le noeud possède des arcs entrants, et on met à jour l'environnement en fonction *)
            ...
            | arcs -> 
              begin
                ...
                let compute_env_with_arc : arc -> env * bool = fun arc0 ->
                  ...
                  let () = if !display then Format.fprintf fmt "Traitement de l'arc %i \n" arc0.arc_id in
                  if not (is_reachable.(arc0.arc_src.node_id)) then begin
                    let () = if !display then Format.fprintf fmt "le noeud source n'est pas accessible. \n" in
                    D.bottom, false
                  end else begin
                    let previous_env = Est.find arc0.arc_src est in (* environnement du noeud source de l'arc *)
                    let () = if !display then Format.fprintf fmt "environnement du noeud source : \n" in
                    let () = if !display then D.print fmt previous_env in

                    let new_current_env, boolean = match arc0.arc_inst with (* nouvel environnement obtenu au noeud courant *)
                      ...
                      | CFG_assert _ -> failwith "CFG_assert not implemented in compute_env_with_arc"
                      | CFG_call _ -> failwith "CFG_call not implemented in compute_env_with_arc"
                    in
                    let () = if !display then Format.fprintf fmt "environnement calculé au noeud courant avec l'arc %i : \n" arc0.arc_id in
                    let () = if !display then D.print fmt new_current_env in
                    new_current_env, boolean
                  end
                ...
                let () = if (!display_minimal || !display) && (not is_it_reachable)
                  then Format.fprintf fmt "le noeud courant est innaccessible \n" in
                let () = if is_reachable.(current_node.node_id) <> is_it_reachable then updated := true in
                let () = if !display_minimal then Format.fprintf fmt "%b -> %b (%b, %b)\n" is_reachable.(current_node.node_id) is_it_reachable (is_reachable.(current_node.node_id) <> is_it_reachable) !updated in
                let () = is_reachable.(current_node.node_id) <- is_it_reachable in
                ...
                let () =
                  if D.is_equal current_env new_current_env && not (!updated) then begin
                    let () = if !display_minimal || !display then Format.fprintf fmt "l'environnement n'a pas été mis à jour \n" in ()
                  end else begin 
                    let () = if !display_minimal || !display then Format.fprintf fmt "l'environnement à été mis à jour \nnoeuds salis : \n" in
                    let () = List.iter 
                      (
                        fun arc ->
                          let dst = arc.arc_dst in
                          let () = if !display_minimal || !display then Format.fprintf fmt "  -> %i \n" dst.node_id in
                          if not is_dirty.(dst.node_id) then Queue.push arc.arc_dst dirty
                      )
                      current_node.node_out
                    in
                    let () = if !display then Format.fprintf fmt "\n" in ()
                  end
                in
                let () = if !display then Format.fprintf fmt "état de l'environnement sur le noeud courant : %a\n" D.print new_current_env in
